...
class HomeNewVisitorTest(StaticLiveServerTestCase):
    def setUp(self):
        activate('en') # activating the language settings otherwise you get 404.
    ...
    # If the title exists then the template has been loaded correctly.
    def test_home_title(self):
        self.browser.get(self.get_full_url("home"))
        self.assertIn("Homepage", self.browser.title)
 
    # If the css test passes, staticfiles are loaded correctly.
    ...
    def test_internationalization(self):
        for lang, h1_text in [('en', 'Welcome'), ('tr', 'Hosgeldiniz')]:
            activate(lang)
            self.browser.get(self.get_full_url("home"))
            h1 = self.browser.find_element_by_tag_name("h1")
